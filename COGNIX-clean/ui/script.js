...
const CONFIG = {
  ...
  USE_DEMO_FALLBACK: false          // false = show backend errors instead of demo replies
};
...
function renderStats() {
  ...
  document.querySelectorAll(".streak-dots i").forEach((dot, i) => dot.classList.toggle("done", i < state.streak));
}
...
async function getAssistantReply(text, files = attachedFiles) {
  try {
    // COGNIX engine integration: route the chat through the diagnostic engine (see cognix-bridge.js).
    if (window.CognixBridge) return await window.CognixBridge.reply(text, files);
...
async function sendMessage() {
  ...
  try {
    const reply = await getAssistantReply(text || "Please analyze the attached file(s).", sentFiles);
...
function switchPage(page) {
  ...
  if (page === "progress") loadBackendData();
...
async function loadBackendData() {
  try {
    const data = await apiFetch(`${CONFIG.STREAK_ENDPOINT}?date=${localDate()}`);
  ...
  try {
    ...
    if (typeof data.questions === "number") state.questions = data.questions;
  } catch (_) {}
  try {
    const data = await apiFetch(CONFIG.NOTES_ENDPOINT);
    if (typeof data.notes === "string" && data.notes && document.activeElement !== $("notesEditor")) {
      $("notesEditor").value = data.notes;
      localStorage.setItem("cognix_notes", data.notes);
    }
  } catch (_) {}
  saveState();
  renderStats();
