...
sqlite.exec(fs.readFileSync(path.join(root, 'backend/db/migrations/002_study.sql'), 'utf8'));
...
const env = {
  ...
  OPENROUTER_SITE_URL: process.env.OPENROUTER_SITE_URL || 'http://localhost:8765',
  OPENROUTER_APP_NAME: process.env.OPENROUTER_APP_NAME || 'COGNIX'
};
...
const staticFiles = {
  ...
  '/lab': ['frontend/index.html', 'text/html; charset=utf-8'],
  '/ui/': ['ui/index.html', 'text/html; charset=utf-8'],
  '/ui/styles.css': ['ui/styles.css', 'text/css; charset=utf-8'],
  '/ui/script.js': ['ui/script.js', 'text/javascript; charset=utf-8'],
  '/ui/cognix-bridge.js': ['ui/cognix-bridge.js', 'text/javascript; charset=utf-8']
};
...
function serveStatic(url, response) {
  ...
  response.writeHead(200, {
    'Content-Type': type,
    'Cache-Control': url.startsWith('/ui/') || url === '/lab' ? 'no-cache' : 'public, max-age=3600',
...
const server = createServer(async (request, response) => {
  try {
    ...
    const maxBody = request.url?.startsWith('/api/transcribe') ? 3_200_000 : request.url?.startsWith('/api/notes') ? 250_000 : 15000;
    const chunks = [];
    let size = 0;
