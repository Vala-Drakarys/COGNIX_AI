...
export function validatePlan(value) {
  if (!plainObject(value)) return false;
  if (Object.keys(value).some(k => !['hypothesis','question_id','question','rationale'].includes(k))) return false;
  return HYPOTHESES.has(value.hypothesis)
    ...
    && typeof value.rationale === 'string' && value.rationale.length > 0 && value.rationale.length <= 400;
}
